...
class StageSeven: StageGameScene {
    ...
    {
        super.addArrowPanel()
        panel.setArrowNum(6, bomb: 2, flappy: 2, split: 2, ignore: 1)
    }

    
    override func restartGame() {
        let gameScene = StageSeven(size: self.size, mainmenu: self.mainmenu, localPlayer: "temp", multiPlayerON: false, selectionScene : self.selectionScene, stage: self.stage)
